...
create sequence if not exists visa_case_id_seq;

create table if not exists visa_case
(
    id                bigint                   default nextval('visa_case_id_seq'::regclass) not null
        constraint visa_case_pk
            primary key,
...
(
    ...
    region       varchar                                                  not null,
    city         varchar                                                  not null,
    visa_case_id bigint                                                   not null
        constraint visit_address_visa_case_id_fk
            references visa_case (id)
);

create sequence if not exists external_inquiry_response_id_seq;

create table if not exists external_inquiry_response
(
    id                      bigint default nextval('external_inquiry_response_id_seq'::regclass) not null
        constraint external_inquiry_response_pk
            primary key,
    from_value              varchar                                                              not null,
    inquiry_response_status varchar                                                              not null,
...
(
    id            bigint                   default nextval('user_task_id_seq'::regclass) not null
        constraint user_task_pk
            primary key,
    visa_case_id  bigint                                                                 not null
        constraint user_task_visa_case_id_fk
            references visa_case (id),
    create_dttm   timestamp with time zone default now()                                 not null,
    is_resolved   boolean                  default false                                 not null,
    comment_value varchar,
    resolved_dttm timestamp with time zone
)
